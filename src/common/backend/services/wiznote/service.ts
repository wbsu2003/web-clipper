...
export default class WizNoteDocumentService implements DocumentService {
  ...
  getUserInfo = async () => {
    if (!this.userInfo) {
      ...
      // 检查 API 返回的错误码
      if ((response as any).returnCode && (response as any).returnCode !== 200) {
        throw new Error(
          `登录失败: ${(response as any).returnMessage || '未知错误'}`
        );
      }

      if (!response.result) {
        throw new Error('登录失败，服务器未返回用户信息，请检查邮箱和密码是否正确');
      }

      if (!response.result.token) {
        throw new Error('登录失败，未获取到 token，请检查账号状态');
      }

      if (!response.result.kbGuid) {
        throw new Error('登录失败，未获取到知识库 ID (kbGuid)');
      }

      this.userInfo = response.result;
    }
    return {
      name: this.userInfo.displayName || this.userInfo.email || '未知用户',
  ...
  getRepositories = async (): Promise<Repository[]> => {
    ...
    if (!this.userInfo?.kbGuid) {
      throw new Error('无法获取知识库信息，kbGuid 不存在');
    }

    let response: WizNoteGetRepositoriesResponse;
    try {
      response = await this.request<WizNoteGetRepositoriesResponse>(
        `/ks/category/all/${this.userInfo.kbGuid}`
      );
    } catch (e: any) {
      throw new Error(`获取知识库列表失败: ${e.message || '网络请求错误'}`);
    }

    if (!response) {
      throw new Error('获取知识库列表失败: 服务器未返回数据');
    }

    // 处理 API 返回错误码的情况
    if ((response as any).returnCode && (response as any).returnCode !== 200) {
      throw new Error(
        `获取知识库列表失败: ${(response as any).returnMessage || '未知错误'}`
      );
    }
